-- Creating and using the New Database
...
-- Creating the supplier_pricing table
...
-- Query 3 : Retrieving Data in the Summarized Format as per the requirements
-- Requirement 1 : Display the total number of customers based on gender who have placed individual orders worth at least Rs.3000
...
-- Requirement 2 : Display all the orders along with the product name ordered by a customer with Customer_Id = 2
...
-- Requirement 3 : Display the Supplier details who can supply more than one product
...
-- Requirement 4 : Find the least expensive product from each category and print the table with category id, name, product name and price of the product
...
WHERE (p.CAT_ID, sp.SUPP_PRICE) IN (
    SELECT p.CAT_ID, MIN(sp.SUPP_PRICE)
    FROM product p
    JOIN supplier_pricing sp ON p.PRO_ID = sp.PRO_ID
    GROUP BY p.CAT_ID
);

-- Requirement 5 : Display the Id and Name of the Product ordered after “2021-10-05”
...
-- Requirement 6 : Display customer name and gender whose names start or end with character 'A'
SELECT CUS_NAME, CUS_GENDER
FROM customer
WHERE CUS_NAME LIKE 'A%' OR CUS_NAME LIKE '%A';

-- Requirement 7 : Create a stored procedure to display supplier id, name, Rating(Average rating of all the products sold by every customer) and Type_of_Service. For Type_of_Service, If rating =5, print “Excellent Service”,If rating >4 print “Good Service”, If rating >2 print “Average Service” else print “Poor Service”. Note that there should be one rating per supplier
DELIMITER //
